...
def check_drinks(drink_name, drink_ingredients):
    ...
    """
    sufix = 'Cocktail' if len(ALCOHOLS & set(drink_ingredients)) else 'Mocktail'
    return drink_name + ' ' + sufix
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    """
    diet_category = (VEGAN, VEGETARIAN, KETO, PALEO, OMNIVORE)
    diet_names = ('VEGAN', 'VEGETARIAN', 'KETO', 'PALEO', 'OMNIVORE')

    for index, category in enumerate(diet_category):
        if dish_ingredients.issubset(category):
            result = dish_name + ': ' + diet_names[index]
            break
...
def singleton_ingredients(dishes, intersection):
    ...
    Each `<CATEGORY>_INTERSECTION` is an `intersection` of all dishes in
    the category. `<CATEGORY>` can be any one of:
